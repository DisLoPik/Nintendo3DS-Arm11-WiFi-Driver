/*
 * 3DS Wi-Fi SDIO driver, shared interface (ARM9 <-> ARM11). The chip is an
 * Atheros AR6014 on a 16-bit TMIO/SDHC host controller ("controller 2", logical
 * 0x10122000) reached from the ARM11. The probe runs in the ARM11 core
 * (src/wifi_arm11.c) and reports back through the shared block below. See
 * docs/wifi.md for the full state of the bring-up.
 */
#ifndef WIFI_H
#define WIFI_H
...
/* ARM11 side (src/wifi_arm11.c): run the full probe, publishing to WifiShared.
 * The host must call this on the ARM11 in response to wifi_host_post_probe(). */
void wifi_probe_run(void);

/* ARM9 side (src/wifi_arm9.c). */
void wifi_probe(void);          /* trigger the ARM11 probe */
